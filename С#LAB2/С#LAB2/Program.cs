...
public class Program
{
    ...
    public static void TaskB(List<Student> students)
    {
        var topStudents = students
            .Where(student => student.Ratings.Any(rating => rating.Subject == "Вища математика"))//відфільтровує студентів
            .OrderByDescending(student =>student.Ratings.Where(rating => rating.Subject == "Вища математика")
            .Average(rating => rating.Module1Score))
            .Select(student => student.LastName);

        Console.WriteLine("Студенти з найвищим середнiм рейтингом за перший модуль:");
    ...
    public static void TaskC(List<Student> students)
    {
        var groupSubjectAverage = students
            .SelectMany(student => student.Ratings, (student, rating) => new { student, rating })//об'єднує оцінки всіх студентів з їх групами та предметами
            .GroupBy(x => new { x.student.Group, x.rating.Subject })//групує елементи з попередньої послідовності
            .Select(group => new//проектує результати групування в новий анонімний тип даних
            {
        ...
        Console.WriteLine("Список з середнiми рейтингами для груп з предмету:");
